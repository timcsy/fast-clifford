...
class CGA0DCareLayer(nn.Module):
    ...
    - M is a motor (2 components: Grade 0, 2)
    ...
    Example:
        ...
        >>> output = layer(motor, point)  # shape: (batch_size, 2)
    """
...
class CGA0DTransformPipeline(nn.Module):
    ...
    Example:
        ...
        >>> motor = torch.randn(batch_size, 2)
        >>> x_0d = torch.randn(batch_size, 0)
        >>> y_0d = pipeline(motor, x_0d)  # shape: (batch_size, 0)
    """
    ...
    def forward(self, motor: Tensor, x: Tensor) -> Tensor:
        """
        Apply motor transformation to 0D point.
