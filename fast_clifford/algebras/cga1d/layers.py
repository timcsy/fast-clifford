...
class CGA1DCareLayer(nn.Module):
    ...
    - M is a motor (4 components: Grade 0, 2)
    ...
    Example:
        ...
        >>> motor = torch.randn(batch_size, 4)
        >>> point = torch.randn(batch_size, 3)
        >>> output = layer(motor, point)  # shape: (batch_size, 3)
    """
...
class CGA1DTransformPipeline(nn.Module):
    ...
    Example:
        ...
        >>> motor = torch.randn(batch_size, 4)
        >>> x_1d = torch.randn(batch_size, 1)
        >>> y_1d = pipeline(motor, x_1d)  # shape: (batch_size, 1)
    """
    ...
    def forward(self, motor: Tensor, x: Tensor) -> Tensor:
        """
        Apply motor transformation to 1D point.
