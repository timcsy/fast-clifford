"""
Clifford Algebra Cl(0, 1) Functional Operations - Auto-generated
...
INNER_PRODUCT_SIGNS = (1, -1)

# Pseudoscalar info
PSEUDOSCALAR_INDEX = 1
...
def geometric_product(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute geometric product of two multivectors in Cl(0, 1).

    Args:
        a: Left operand, shape (..., 2)
        b: Right operand, shape (..., 2)

    Returns:
        Result multivector, shape (..., 2)
    """
    r0 = a[..., 0] * b[..., 0] + -a[..., 1] * b[..., 1]
    r1 = a[..., 0] * b[..., 1] + a[..., 1] * b[..., 0]
...
def reverse(mv: Tensor) -> Tensor:
    """
    Compute reverse of a multivector in Cl(0, 1).

    For grade k: coefficient *= (-1)^(k*(k-1)/2)

    Args:
        mv: Input multivector, shape (..., 2)

    Returns:
        Reversed multivector, shape (..., 2)
    """
    r0 = mv[..., 0]
    r1 = mv[..., 1]
...
def involute(mv: Tensor) -> Tensor:
    """
    Compute grade involution of a multivector in Cl(0, 1).
    ...
    Returns:
        Grade-involuted multivector, shape (..., 2)
    """
    r0 = mv[..., 0]
    r1 = -mv[..., 1]
...
def conjugate(mv: Tensor) -> Tensor:
    """
    Compute Clifford conjugate of a multivector in Cl(0, 1).
    ...
    Returns:
        Conjugated multivector, shape (..., 2)
    """
    r0 = mv[..., 0]
    r1 = -mv[..., 1]
...
def select_grade_0(mv: Tensor) -> Tensor:
    ...
    Returns:
        Multivector with only grade-0 components, shape (..., 2)
    """
    r0 = mv[..., 0]
...
def select_grade_1(mv: Tensor) -> Tensor:
    ...
    Returns:
        Multivector with only grade-1 components, shape (..., 2)
    """
...
def inner(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute inner product <ab>_0 in Cl(0, 1).
...
def outer(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute outer product a ^ b in Cl(0, 1).
...
def contract_left(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute left contraction a << b in Cl(0, 1).
...
def contract_right(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute right contraction a >> b in Cl(0, 1).
...
def dual(mv: Tensor) -> Tensor:
    """
    Compute dual mv* = mv << I in Cl(0, 1).

    Args:
        mv: Input multivector, shape (..., 2)

    Returns:
        Dual multivector, shape (..., 2)
    """
    r0 = -mv[..., 1]
    r1 = mv[..., 0]
...
def norm_squared(mv: Tensor) -> Tensor:
    """
    Compute norm squared |mv|^2 = <mv * ~mv>_0 in Cl(0, 1).
...
def compose_rotor(r1: Tensor, r2: Tensor) -> Tensor:
    """
    Compose two rotors r1 * r2 in Cl(0, 1).
...
def reverse_rotor(r: Tensor) -> Tensor:
    """
    Compute reverse of a rotor in Cl(0, 1).
...
def sandwich_rotor(r: Tensor, x: Tensor) -> Tensor:
    """
    Compute sandwich product r * x * ~r in Cl(0, 1).

    Args:
        r: Rotor, shape (..., 1)
        x: Operand multivector, shape (..., 2)

    Returns:
        Transformed multivector, shape (..., 2)
...
def norm_squared_rotor(r: Tensor) -> Tensor:
    """
    Compute norm squared of a rotor in Cl(0, 1).
...
def exp_bivector(B: Tensor) -> Tensor:
    """
    Compute exponential of a bivector in Cl(0, 1).
...
def log_rotor(r: Tensor) -> Tensor:
    """
    Compute logarithm of a rotor in Cl(0, 1).
...
def slerp_rotor(r1: Tensor, r2: Tensor, t: Tensor) -> Tensor:
    """
    Spherical linear interpolation between rotors in Cl(0, 1).
