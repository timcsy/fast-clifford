"""
Clifford Algebra Cl(0, 2) Functional Operations - Auto-generated
...
INVOLUTE_SIGNS = (1, -1, -1, 1)

# Conjugate signs for all blades
CONJUGATE_SIGNS = (1, -1, -1, -1)

# Inner product signs (blade² values)
INNER_PRODUCT_SIGNS = (1, -1, -1, -1)

# Pseudoscalar info
PSEUDOSCALAR_INDEX = 3
...
def geometric_product(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute geometric product of two multivectors in Cl(0, 2).
...
def reverse(mv: Tensor) -> Tensor:
    """
    Compute reverse of a multivector in Cl(0, 2).
...
def involute(mv: Tensor) -> Tensor:
    """
    Compute grade involution of a multivector in Cl(0, 2).
    ...
    Returns:
        Grade-involuted multivector, shape (..., 4)
    """
    r0 = mv[..., 0]
    r1 = -mv[..., 1]
    r2 = -mv[..., 2]
...
def conjugate(mv: Tensor) -> Tensor:
    """
    Compute Clifford conjugate of a multivector in Cl(0, 2).
    ...
    Returns:
        Conjugated multivector, shape (..., 4)
    """
    r0 = mv[..., 0]
    r1 = -mv[..., 1]
    r2 = -mv[..., 2]
...
def select_grade_0(mv: Tensor) -> Tensor:
    ...
    Returns:
        Multivector with only grade-0 components, shape (..., 4)
    """
    r0 = mv[..., 0]
...
def select_grade_1(mv: Tensor) -> Tensor:
    ...
    Returns:
        Multivector with only grade-1 components, shape (..., 4)
    """
...
def select_grade_2(mv: Tensor) -> Tensor:
    ...
    Returns:
        Multivector with only grade-2 components, shape (..., 4)
    """
    r0 = torch.zeros_like(mv[..., 0])
    r1 = torch.zeros_like(mv[..., 1])
    r2 = torch.zeros_like(mv[..., 2])
...
def inner(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute inner product <ab>_0 in Cl(0, 2).
...
def outer(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute outer product a ^ b in Cl(0, 2).
...
def contract_left(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute left contraction a << b in Cl(0, 2).
...
def contract_right(a: Tensor, b: Tensor) -> Tensor:
    """
    Compute right contraction a >> b in Cl(0, 2).
...
def dual(mv: Tensor) -> Tensor:
    """
    Compute dual mv* = mv << I in Cl(0, 2).
...
def norm_squared(mv: Tensor) -> Tensor:
    """
    Compute norm squared |mv|^2 = <mv * ~mv>_0 in Cl(0, 2).
    ...
    """
    result = mv[..., 0] * mv[..., 0] + -mv[..., 1] * mv[..., 1] + -mv[..., 2] * mv[..., 2] + mv[..., 3] * mv[..., 3]
    return result.unsqueeze(-1)
...
def compose_rotor(r1: Tensor, r2: Tensor) -> Tensor:
    """
    Compose two rotors r1 * r2 in Cl(0, 2).

    Args:
        r1: Left rotor, shape (..., 2)
        r2: Right rotor, shape (..., 2)

    Returns:
        Composed rotor, shape (..., 2)
    """
    out0 = r1[..., 0] * r2[..., 0] + -r1[..., 1] * r2[..., 1]
    out1 = r1[..., 0] * r2[..., 1] + r1[..., 1] * r2[..., 0]
...
def reverse_rotor(r: Tensor) -> Tensor:
    """
    Compute reverse of a rotor in Cl(0, 2).

    Args:
        r: Input rotor, shape (..., 2)

    Returns:
        Reversed rotor, shape (..., 2)
    """
    r0 = r[..., 0]
    r1 = -r[..., 1]
...
def sandwich_rotor(r: Tensor, x: Tensor) -> Tensor:
    """
    Compute sandwich product r * x * ~r in Cl(0, 2).

    Args:
        r: Rotor, shape (..., 2)
        x: Operand multivector, shape (..., 4)

    Returns:
        Transformed multivector, shape (..., 4)
...
def norm_squared_rotor(r: Tensor) -> Tensor:
    """
    Compute norm squared of a rotor in Cl(0, 2).
...
def log_rotor(r: Tensor) -> Tensor:
    """
    Compute logarithm of a rotor in Cl(0, 2).
...
def slerp_rotor(r1: Tensor, r2: Tensor, t: Tensor) -> Tensor:
    """
    Spherical linear interpolation between rotors in Cl(0, 2).

    Formula: slerp(r1, r2, t) = r1 * exp(t * log(r1~ * r2))

    Args:
        r1: Start rotor, shape (..., 2)
        r2: End rotor, shape (..., 2)
        t: Interpolation parameter [0, 1], shape (...) or scalar

    Returns:
        Interpolated rotor, shape (..., 2)
    """
